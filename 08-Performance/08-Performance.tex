...
\label{sec:Performance}
  ...
  The fitted transverse positions, $(x,y)$, and momenta, $(p_x, p_y, p_z)$, were compared to the the Monte Carlo truth on an event-by-event basis. The Monte Carlo reconstructed events were subject to the same requirements as the real data. The Monte Carlo truth data was provided by the simulated track information, stored at every tracker plane to permit a direct comparison with the reconstructed data. All comparisons were made at the tracker reference serface. %the designated measurement location with the MICE cooling channel.

  An artificial beam was generated with uniform distributions in both the longitudinal and transverse momenta. This was to ensure that the results were not biased by the incoming beam distribution, and that the full reconstructible phase space was probed with equal statistics. In order to remove unwanted non-physical particles, a cut was placed on the true Monte Carlo data during the analysis phase to eliminate tracks with a high incoming angle. Tracks with a gradient ($p_t/p_z$) greater than $0.5$ were rejected from the analysis.
  ...
  \begin{figure}[p]
    ...
    \caption{\label{fig:tp_efficiency} The efficiency of reconstructing trackpoints in the upstream (left) and downstream (right) trackers as a function of the simulated transverse momentum.}
  \end{figure}
  ...
  The track fit assumes a simple model of energy loss based on the mean thickness of materials used in the tracker construction, however the effects of glues and resins, in addition to non-uniform density effects are not considered. This results in systematic underestimate for the energy lost at each tracker plane. Due to the high-granularity of the position information utilised by the fit, the position reconstruction is much less sensitive to this effect. Figure~\ref{fig:pBiasKalman} shows how this affects the final track fit at the reference plane.
  \begin{figure}[h]
    \begin{center}
  ...
  This discrepency is in part due to the accuracy of the pattern recognition parameters, and in part due to the energy loss model and is not trivial to remove. It can be well appoximately using a linear fit, and hence can be accounted for during the analysis phase, under the assumption that the Monte Carlo geometry correctly models the physical tracker.
